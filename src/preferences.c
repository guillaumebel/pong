...
{
	pong_properties_save (properties);
...
difficulty_cb (GtkWidget *widget, gpointer data)
{
	if (GTK_TOGGLE_BUTTON (widget)->active)
		properties->difficulty = GPOINTER_TO_INT (data);
}
...
{
	if (GTK_TOGGLE_BUTTON (widget)->active)
	  properties->paddle_size = GPOINTER_TO_INT (data);
}
